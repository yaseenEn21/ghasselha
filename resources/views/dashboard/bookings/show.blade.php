...
@php
    ...
    $serviceName = $booking->service?->name ? (is_array($booking->service->name) ? ($booking->service->name[$locale] ?? collect($booking->service->name)->first()) : $booking->service->name) : '—';

    // car/address fields (عدّلها حسب أعمدتك)
    $carLabel = data_get($booking, 'car.plate_number')
        ?? data_get($booking, 'car.plate')
        ?? data_get($booking, 'car.number')
        ?? ('#' . (int) $booking->car_id);

    $addressLabel = data_get($booking, 'address.title')
        ?? data_get($booking, 'address.address')
        ?? data_get($booking, 'address.full_address')
        ?? ('#' . (int) $booking->address_id);
    ...
    $timeRange = ($booking->start_time ? substr((string)$booking->start_time,0,5) : '—')
        . ' → ' .
            ...
            <div class="d-flex align-items-center gap-4">
                ...
                <div>
                    <div class="d-flex align-items-center gap-2 mb-1">
                        ...
                        <span class="badge badge-light-{{ $statusClass }}">{{ __('bookings.status.' . $statusKey) }}</span>
                    </div>
                    <div class="text-muted fw-semibold">
                        {{ $booking->booking_date?->format('Y-m-d') }} • {{ $timeRange }} • {{ $tpLabel }}
                        • {{ __('bookings.duration') }}: {{ (int) $booking->duration_minutes }} {{ __('bookings.minutes') }}
                    </div>
                </div>
            </div>
                            ...
                            <div class="card-body pt-0">
                                <div class="d-flex flex-column gap-3">
                                    ...
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <span class="fw-semibold">{{ $booking->zone_id ? ('#' . (int)$booking->zone_id) : '—' }}</span>
                                    </div>
                            ...
                            <div class="card-body pt-0">
                                ...
                                <div class="d-flex flex-column gap-3">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <span class="fw-semibold">{{ __('bookings.pricing_source_values.' . ($booking->service_pricing_source ?? 'base')) }}</span>
                                    </div>

                                    <div class="d-flex justify-content-between">
                                        <span class="text-muted">{{ __('bookings.service_unit_price') }}</span>
                                        <span class="fw-semibold">{{ number_format((float)$booking->service_unit_price_snapshot, 2) }} {{ $booking->currency }}</span>
                                    </div>

                                    <div class="d-flex justify-content-between">
                                        <span class="text-muted">{{ __('bookings.service_charge_amount') }}</span>
                                        <span class="fw-semibold">{{ number_format((float)$booking->service_charge_amount_snapshot, 2) }} {{ $booking->currency }}</span>
                                    </div>
                                    ...
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <span class="fw-semibold">{{ number_format((float)$booking->subtotal_snapshot, 2) }} {{ $booking->currency }}</span>
                                    </div>
                                    <div class="d-flex justify-content-between">
                                        <span class="text-muted">{{ __('bookings.discount') }}</span>
                                        <span class="fw-semibold">{{ number_format((float)$booking->discount_snapshot, 2) }} {{ $booking->currency }}</span>
                                    </div>
                                    <div class="d-flex justify-content-between">
                                        <span class="text-muted">{{ __('bookings.tax') }}</span>
                                        <span class="fw-semibold">{{ number_format((float)$booking->tax_snapshot, 2) }} {{ $booking->currency }}</span>
                                    </div>
                                    <div class="d-flex justify-content-between">
                                        <span class="text-muted fw-bold">{{ __('bookings.total') }}</span>
                                        <span class="fw-bold">{{ number_format((float)$booking->total_snapshot, 2) }} {{ $booking->currency }}</span>
                                    </div>
                                </div>

                                @if(!empty($booking->service_pricing_meta))
                                    <div class="mt-6">
                                        <div class="text-muted fw-semibold mb-2">{{ __('bookings.pricing_meta') }}</div>
                                ...
                                <div class="row g-6">
                                    <div class="col-md-4">
                                        <div class="d-flex flex-column">
                                            ...
                                            <span class="fw-semibold">{{ $booking->created_at?->format('Y-m-d H:i') ?? '—' }}</span>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="d-flex flex-column">
                                            <span class="text-muted">{{ __('bookings.confirmed_at') }}</span>
                                            <span class="fw-semibold">{{ $booking->confirmed_at?->format('Y-m-d H:i') ?? '—' }}</span>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="d-flex flex-column">
                                            <span class="text-muted">{{ __('bookings.cancelled_at') }}</span>
                                            <span class="fw-semibold">{{ $booking->cancelled_at?->format('Y-m-d H:i') ?? '—' }}</span>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div class="d-flex flex-stack flex-grow-1">
                                            <div class="fw-semibold">
                                                <div class="fs-6 text-gray-800">
                                                    <div class="fw-bold mb-1">{{ __('bookings.cancel_reason') }}: {{ $booking->cancel_reason ?? '—' }}</div>
            ...
            <div class="tab-pane fade" id="tab_products" role="tabpanel">
                ...
                <div class="table-responsive">
                    <table class="table align-middle table-row-dashed fs-6 gy-4">
                        ...
                        @forelse($booking->products as $i => $p)
                            @php
                                $title = $p->title ? (is_array($p->title) ? ($p->title[$locale] ?? collect($p->title)->first()) : $p->title) : null;
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="text-muted fs-7">{{ __('bookings.products.product_id') }}: #{{ $p->product_id }}</div>
                                </td>
                                <td>{{ (int)$p->qty }}</td>
                                <td>{{ number_format((float)$p->unit_price_snapshot, 2) }} {{ $booking->currency }}</td>
                                <td class="fw-bold">{{ number_format((float)$p->line_total, 2) }} {{ $booking->currency }}</td>
                            </tr>
                        @empty
                            <tr>
                ...
                <div class="d-flex justify-content-end mt-6">
                    <div class="d-flex flex-column text-end">
                        ...
                        <span class="fs-4 fw-bold">{{ number_format((float)$booking->products_subtotal_snapshot, 2) }} {{ $booking->currency }}</span>
                    </div>
                </div>
                        ...
                        <div class="d-flex flex-stack flex-grow-1">
                            <div class="fw-semibold">
                                <div class="fs-6 text-gray-800">
                                    ...
                                    <span class="fw-bold ms-2">#{{ $latestUnpaid->id }} ({{ $latestUnpaid->number ?? '—' }})</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="timeline-item">
                            ...
                            <div class="timeline-content ps-3">
                                <div class="fw-semibold">
                                    ...
                                    <span class="badge badge-light">{{ $log->from_status ? __('bookings.status.' . $log->from_status) : '—' }}</span>

                                    <span class="mx-2">→</span>

                                    {{ __('bookings.logs.to') }}:
                                    <span class="badge badge-light-primary">{{ __('bookings.status.' . $log->to_status) }}</span>
                                </div>
                                ...
                                @if($log->created_by)
                                    <div class="text-muted fs-7 mt-1">{{ __('bookings.logs.by') }}: #{{ (int)$log->created_by }}</div>
                                @endif
                            </div>
                        </div>
                    @empty
